...
// #define SENSOR_INTERVAL_SEC	TIMER_TICKS_PER_SEC * 15 // every 30 seconds
// #define SENSOR_INTERVAL_SEC	TIMER_TICKS_PER_SEC * 10 // every 20 seconds
// #define SENSOR_INTERVAL_SEC	TIMER_TICKS_PER_SEC * 6 // every 12 seconds
// #define SENSOR_INTERVAL_SEC	TIMER_TICKS_PER_SEC * 4 // every 8 seconds
 //#define SENSOR_INTERVAL_SEC	TIMER_TICKS_PER_SEC * 3 // every 6 seconds
 //#define SENSOR_INTERVAL_SEC	TIMER_TICKS_PER_SEC * 2 // every 4 seconds
 #define SENSOR_INTERVAL_SEC TIMER_TICKS_PER_SEC  // every 2 seconds
...
void on_alp_command_completed_cb(uint8_t tag_id, bool success)
{
    ...
    */
    ...
    // infinite loop and one push
    timer_post_task_delay(&execute_sensor_measurement, (SENSOR_INTERVAL_SEC/10));

    //infinite loop every 0.2 (2/10) seconds
    //timer_post_task_delay(&execute_sensor_measurement, (SENSOR_INTERVAL_SEC/10));
...
void bootstrap()
{
     ...
     // send 10 pushes and infinite (ok)
   // timer_post_task_delay(&execute_sensor_measurement, (SENSOR_INTERVAL_SEC)); NOT USED
