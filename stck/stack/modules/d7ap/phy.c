...
static channel_id_t default_channel_id = {
  ...
  .channel_header.ch_class = PHY_CLASS_LO_RATE, // was NORMAL
  .channel_header.ch_freq_band = PHY_BAND_868,  // was PHY_BAND_868
  .center_freq_index = 0
};
...
static void configure_channel(const channel_id_t* channel) {
    ...
    if(channel->channel_header.ch_freq_band == PHY_BAND_868)
        center_freq = 863e6; // defaut 863e6 - between 863e6 - 870e6, according to D7A spec 1.2
    else if(channel->channel_header.ch_freq_band == PHY_BAND_915)
        center_freq = 902e6;
    else if(channel->channel_header.ch_freq_band == PHY_BAND_433) // added
        center_freq = 433.060e6; //  defaut 433.060e6 - between 433.060 – 434.785 MHz, according to D7A spec 1.2 (added)
